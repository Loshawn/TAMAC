...
from utils import sample_box_pose, sample_insertion_pose, sample_stack_pose, sample_storage_pose  # robot functions
from utils import compute_dict_mean, set_seed, detach_dict  # helper functions
from policy import ACTPolicy, CNNMLPPolicy, TAMACPolicy
...
def main(args):
    ...
    if is_eval:
        ...
        for ckpt_name in ckpt_names:
            # success_rate, avg_return = eval_bc(config,
            #                                    ckpt_name,
            #                                    save_episode=True)
    ...
    train_dataloader, val_dataloader, stats, _ = load_data(policy_class, dataset_dir, num_episodes,
                                                           camera_names, batch_size_train,
...
def make_policy(policy_class, policy_config):
    if policy_class == 'ACT':
        ...
        policy = TAMACPolicy(policy_config)
    else:
        raise NotImplementedError
    return policy
...
def eval_bc(config, ckpt_name, save_episode=True):
    ...
    for rollout_id in range(num_rollouts):
        ...
        ### evaluation loop
        if temporal_agg:
            all_time_actions = torch.zeros([max_timesteps, max_timesteps + num_queries,
                                            state_dim]).cuda()
        ...
        with torch.inference_mode():
            for t in tqdm(range(max_timesteps)):
                ...
                elif config['policy_class'] == "TAMAC":
                    # update input queue
                    if t == 0:
                        image_queue = [curr_image] + [torch.zeros_like(curr_image)] * 49
                        qpos_queue = [qpos] + [torch.zeros_like(qpos)] * 49
                    else:
                        image_queue.pop(-1)
                        image_queue.insert(0, curr_image)
                        qpos_queue.pop(-1)
                        qpos_queue.insert(0, qpos)

                    if t % query_frequency == 0:
                        image_tensor = torch.cat(image_queue[::10],
                                                 dim=1).cuda()  # (1, 5, 3, 480, 640)
                        qpos_tensor = torch.stack(qpos_queue, dim=1).cuda()  # (1, 50, 14)
        ...
        print(
            f'{episode_return=}, {episode_highest_reward=}, {env_max_reward=}, Success: {episode_highest_reward==env_max_reward}'
        )
...
def forward_pass(data, policy):
    ...
    image_data, qpos_data, action_data, is_pad = image_data.cuda(), qpos_data.cuda(
    ), action_data.cuda(), is_pad.cuda()
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        epoch_summary = compute_dict_mean(train_history[(batch_idx + 1) * epoch:(batch_idx + 1) *
                                                        (epoch + 1)])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_dir',
                        default=f"ckpt/{policy}/{task}/debug",
                        action='store',
                        type=str,
                        help='ckpt_dir',
                        required=False)

    parser.add_argument('--policy_class',
                        default=f"{policy}",
                        action='store',
                        type=str,
    ...
    parser.add_argument('--use_pos_embd_action',
                        action='store',
                        type=int,
                        default=1,
                        required=False)
    ...
    # for TAMAC
    parser.add_argument('--use_full_seq', action='store_false')
